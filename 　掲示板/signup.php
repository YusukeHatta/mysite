        ...
        <div class="box-body">
          <form action="user_check.php" method="post">
            <div class="form-group">
              <label for="exampleInputEmail1">登録ユーザーID</label>
              <input type="text" name="userid" value="<?php echo $userid; ?>" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter userid">
              <?php if (isset($error["userid"])) { ?><div style="color:red;"><?php echo $error["userid"] ?></div><?php } ?>
            </div>
            <div class="form-group">
              <label for="exampleInputEmail1">登録パスワード</label>
              <input type="text" name="pass" value="<?php echo $pass; ?>" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter password">
              <?php if (isset($error["pass"])) { ?><div style="color:red;"><?php echo $error["pass"] ?></div><?php } ?>
            </div>
            <button type="submit" class="btn primary">送信内容確認</button>
          </form>
          <form action="login.php" method="post">
          <input class="btn btn-primary"type="submit" value="ログインページへ戻る">
          </form>
